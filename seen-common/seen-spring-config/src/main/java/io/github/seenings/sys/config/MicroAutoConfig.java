package io.github.seenings.sys.config;

import io.github.seenings.sys.constant.ServiceNameConstant;
import lombok.AllArgsConstructor;
import lombok.extern.slf4j.Slf4j;
import org.springframework.boot.autoconfigure.AutoConfiguration;
import org.springframework.context.annotation.Bean;
import org.springframework.web.client.RestClient;
import org.springframework.web.client.support.RestClientAdapter;
import org.springframework.web.service.invoker.HttpServiceProxyFactory;

/**
 * 微服务配置
 */
@Slf4j
@AutoConfiguration
@AllArgsConstructor
public class MicroAutoConfig {
    /**
     * 客户端
     */
    private RestClient.Builder restClientBuilder;

    /**
     * 币的代理
     *
     * @return 代理
     */
    @Bean
    public HttpServiceProxyFactory httpServiceProxyFactoryBySeenCoin() {
        RestClient restClient = restClientBuilder.baseUrl("http://" + ServiceNameConstant.SERVICE_SEEN_COIN).build();
        return HttpServiceProxyFactory.builderFor(RestClientAdapter.create(restClient)).build();
    }

    /*
     * 用户服务的代理
     *
     * @return 代理
     */
    @Bean
    public HttpServiceProxyFactory httpServiceProxyFactoryBySeenUser() {
        RestClient restClient = restClientBuilder.baseUrl("http://" + ServiceNameConstant.SERVICE_SEEN_USER).build();
        return HttpServiceProxyFactory.builderFor(RestClientAdapter.create(restClient)).build();
    }

    /*
     * 业务的代理
     *
     * @return 代理
     */
    @Bean
    public HttpServiceProxyFactory httpServiceProxyFactoryBySeenBusi() {
        RestClient restClient = restClientBuilder.baseUrl("http://" + ServiceNameConstant.SERVICE_SEEN_BUSI).build();
        return HttpServiceProxyFactory.builderFor(RestClientAdapter.create(restClient)).build();
    }

    /*
     * 计算的代理
     *
     * @return 代理
     */
    @Bean
    public HttpServiceProxyFactory httpServiceProxyFactoryBySeenCalc() {
        RestClient restClient = restClientBuilder.baseUrl("http://" + ServiceNameConstant.SERVICE_SEEN_CALC).build();
        return HttpServiceProxyFactory.builderFor(RestClientAdapter.create(restClient)).build();
    }

    /*
     * 文章服务
     *
     * @return 代理
     */
    @Bean
    public HttpServiceProxyFactory httpServiceProxyFactoryBySeenArticle() {
        RestClient restClient = restClientBuilder.baseUrl("http://" + ServiceNameConstant.SERVICE_SEEN_ARTICLE).build();
        return HttpServiceProxyFactory.builderFor(RestClientAdapter.create(restClient)).build();
    }

    /*
     * 套接字
     *
     * @return 代理
     */
    @Bean
    public HttpServiceProxyFactory httpServiceProxyFactoryBySeenSocket() {
        RestClient restClient = restClientBuilder.baseUrl("http://" + ServiceNameConstant.SERVICE_SEEN_SOCKET).build();
        return HttpServiceProxyFactory.builderFor(RestClientAdapter.create(restClient)).build();
    }

    /*
     * 中间结果
     *
     * @return 代理
     */
    @Bean
    public HttpServiceProxyFactory httpServiceProxyFactoryBySeenMiddle() {
        RestClient restClient = restClientBuilder.baseUrl("http://" + ServiceNameConstant.SERVICE_SEEN_MIDDLE).build();
        return HttpServiceProxyFactory.builderFor(RestClientAdapter.create(restClient)).build();
    }

    /*
     * 聊天
     *
     * @return 代理
     */
    @Bean
    public HttpServiceProxyFactory httpServiceProxyFactoryBySeenChat() {
        RestClient restClient = restClientBuilder.baseUrl("http://" + ServiceNameConstant.SERVICE_SEEN_CHAT).build();
        return HttpServiceProxyFactory.builderFor(RestClientAdapter.create(restClient)).build();
    }

    /*
     * 其他
     *
     * @return 代理
     */
    @Bean
    public HttpServiceProxyFactory httpServiceProxyFactoryBySeenOther() {
        RestClient restClient = restClientBuilder.baseUrl("http://" + ServiceNameConstant.SERVICE_SEEN_OTHER).build();
        return HttpServiceProxyFactory.builderFor(RestClientAdapter.create(restClient)).build();
    }
    /*
     * 学校
     *
     * @return 代理
     */
    @Bean
    public HttpServiceProxyFactory httpServiceProxyFactoryBySeenSchool() {
        RestClient restClient = restClientBuilder.baseUrl("http://" + ServiceNameConstant.SERVICE_SEEN_SCHOOL).build();
        return HttpServiceProxyFactory.builderFor(RestClientAdapter.create(restClient)).build();
    }
    /*
     * 文件
     *
     * @return 代理
     */
    @Bean
    public HttpServiceProxyFactory httpServiceProxyFactoryBySeenFile() {
        RestClient restClient = restClientBuilder.baseUrl("http://" + ServiceNameConstant.SERVICE_SEEN_FILE).build();
        return HttpServiceProxyFactory.builderFor(RestClientAdapter.create(restClient)).build();
    }
}
