...
public class FreezeServiceImpl implements FreezeService {
     ...
     */
    @Override
    public Boolean checkEnough(Long userId, int coinMount) {
     ...
     */
    @Override
    public Set<Integer> checkEnoughAndFreeze(Long userId, int coinMount, TradeType tradeType, String description) {
        ...
        if (temporaryCoinAmount >= coinMount) {
            ...
            Integer temporaryTradeId = tradeService.trade(freezeAccountId, temporaryAccountId, coinMount, TradeType.APPLY_DO_FRIEND, description);
